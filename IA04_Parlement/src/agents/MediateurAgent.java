...
public class MediateurAgent extends Agent{
	...
	{ 
		...
		addBehaviour(new OneShotBehaviour(){
			...
			public void action() {
				...
				addBehaviour(new TourFromSimulation()); // rec�ption d'un message marquant le d�but d'un nouveau tour de jeu de l'agent simulation (REQUEST)
				// --> Envoie d'un message a AUtilisateur avec la liste des actions possibles

				addBehaviour(new ActionFromUtilisateur()); // r�ception de l'action choisit par l'utilisateur (ACCEPT_PROPOSAL)
				// --> Suivant l'action possibilit� de lui renvoyer un message pour lui demander (loi,personne vis�, etc...)

				addBehaviour(new PrecisionFromUtilisateur()); // r�ception de l'information n�cessaire pour faire l'action. (INFORM)
				// --> Suivant l'action traitement diff�rents.

				addBehaviour(new FinVoteFromLoi()); // r�ception de la fin d'un vote de loi (REQUEST)
				// --> Suivant l'action traitement diff�rents.
		...
		System.out.println("Agent Mediateur cr�� : "+this.getLocalName());
	}
	...
	class TourFromSimulation extends CyclicBehaviour{
		...
		public void action() {
			...
			if (message != null){
				...
				//On r�cup�re la variable tour envoy� par l'agent Simulation et on la stocke
				...
				// On construit la liste des actions a proposer au joueur suivant le num�ro du tour
				L_Actions.clear();
				...
				}else{	// Si ce n'est pas � l'utilisateur de proposer une loi, alors il faut pr�venir l'agent Loi qu'il demande � un d�put� d'en proposer une

					//Envoie d'un message � l'Agent loi pour lancer une proposition de loi par un d�put�
					if (ALoi != null) {
				...
				// Envoie d'un message � l'agent Utilisateur pour qu'il choisisse quelle actions faire.
				if (L_Actions.size() > 1)
	...
	class ProposeActionsToUser extends OneShotBehaviour{
		...
		public void action() {
			...
			// On serialise le message contenant la liste des actions possibles et on l'envoie � l'agent utilisateur
		...
		public void action() {
			...
			if (message != null){
				...
				case "Proposer une loi":
					// Il faut demander � l'utilisateur de pr�ciser quelle loi veut-il proposer.
				...
				case "Faire un sondage":
					// Envoyer une demande � l'agent Sondage.
					if (ASondage != null) {
						ACLMessage message2 = new ACLMessage(ACLMessage.REQUEST);
		...
		public void action() {
			...
			if (message != null){
				...
				case "Proposer une loi":
					...
					System.out.println();
					System.out.println("Vous avez propos� cette loi : ");
					loi.affiche();
					
					//Envoie de la loi choisit par l'utilisateur � l'Agent Loi.
					if (ALoi != null) {
						ACLMessage message2 = new ACLMessage(ACLMessage.REQUEST);
						message2.addReceiver(ALoi);
