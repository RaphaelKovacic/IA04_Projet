 ...
 * Les comportements de l'agent KB sont spécifiés dans les deux classes suivantes
...
public class KB extends Agent{
     ...
     * <p>
     * Lors du lancement de notre plateforme JADE, l'agent KB est créé grâce à cette méthode setup()
     * </p>
     */
	...
	{ 
		...
		addBehaviour(new OneShotBehaviour(){
			...
			addBehaviour(new RequestLaw()); // réception d'un message de type REQUEST avec un nom de parti, on doit répondre avec la liste des lois (ou bien une seule) non encore votées correspondantes 

			addBehaviour(new VotedAckLaw()); // réception d'un message de type INFORM avec un id de loi que l'on doit passer à is_voted = true
	 ...
	 * <p>Il est de type Cyclic. Notre agent KB est en constante attente d'une requête d'un agent extérieur pouvant le soliciter.
	 * </p>
	 * <p>
	 * Il implémente le comportement suivant :
	 * Retourne une loi (au député le requêtant) ou une liste de loi (à l'agent médiateur forwardant le message de l'utilisateur)
	 * <p>
	 * 
	 * @author Etienne
	 * @version 2.1
	...
	class RequestLaw extends CyclicBehaviour{
	     ...
	     * <p>
	     * L'ensemble des tâches réalisées par le Behaviour RequestLaw est contenu ici.
	     * </p>
	     */
		...
		public void action() {
			...
			// Soit il vient du médiateur et on doit renvoyer toutes les lois correspondants à ce party (qui a forwarder le choix de l'utilisateur)
			MessageTemplate mt_mediateur = MessageTemplate.and(
					MessageTemplate.MatchPerformative(ACLMessage.REQUEST),
					MessageTemplate.MatchSender(AMediateur)
					);
			
			//Soit il vient d'un député à qui on doit renvoyer seulement une loi de la liste en JSON
	 ...
	 * <p> Ce second behaviour est de type Cyclic. Notre agent KB est en constante attente d'un acquittement concernant une loi qui viendrait d'être votée.
	 * </p>
	 * <p>
	 * Il implémente le comportement suivant :
	 * Modifie le model JENA pour "désactiver" les lois votées afin que celles-ci ne puissent l'être une et une seule fois.
	 * <p>
	 * 
	 * @author Etienne
	 * @version 1.2
	...
	class VotedAckLaw extends CyclicBehaviour{
		...
		public void action() {
			...
			// On attend la reception d'un message de type INFORM de la part de l'agent loi 
	 ...
	 /**
     * Renvoie une liste de loi disponibles (non encore votées) pour un parti spécifique.
     * 
     * @param _s
     *            Chaîne de caractère correspondant au parti politique dont on cherche les lois disponibles au vote
     * 
     * Retourne une liste d'objet de type Loi correspondant aux lois disponibles au vote pour le parti.
     * 
	...
	public List<Loi> PossibleLaws(String _s){
	 ...
	 /**
     * Renvoie le nom JENA de la loi correspondant à l'id passé en paramètre. 
     * On renvoie le sujet du triplet (sujet_voulu, law:id, _id)
     * 
     * @param _id
     *            La chaine de caractère correspondant à un id de loi.
     * 
     * Retourne le nom JENA dans une ressource correspondant à l'id de la loi voulue.
     * 
     * @return Une ressource JENA correspondant au sujet de l'id de la loi passée en paramètre.
     * 
     */
	...
	public Resource getLoiFromId(String _id){
	 ...
	 /**
     * Renvoie une liste de Statement JENA filtrée sur le nom du parti politique grâce au triple (sujet, law:politic_party, _parti)
     * 
     * @param _parti
     *            Chaîne de caractère correspondant au parti politique dont on cherche les lois disponibles au vote
     * 
     * Retourne une liste de statement correspondant aux triples (loi, law:politic_party, _parti)
     * 
     ...
     * 
     * Retourne un booléen pour connaitre si le loi n'a pas été votée (true) ou votée (false)
     * 
     * @return Un booléen.
     * 
     *  @see getLoiFromParti
	...
	/**
     * Renvoie la chaine de caractère correspondant à la propriété law:id pour un sujet "loi" passé en paramètre.
     * Assimilable à un "getter" JENA sur une propriété donnée avec un sujet passé en argument.
     * 
     * @param _rsc
     *            Une ressource "objet" JENA
     * 
     * Retourne une chaine de caractère correspondant à la valeur de la propriété law:id
     * 
	...
	/**
     * Renvoie la chaine de caractère correspondant à la propriété law:name pour un sujet "loi" passé en paramètre.
     * Assimilable à un "getter" JENA sur une propriété donnée avec un sujet passé en argument.
     * 
     * @param _rsc
     *            Une ressource "objet" JENA
     * 
     * Retourne une chaine de caractère correspondant à la valeur de la propriété law:name
     * 
     * @return Une chaine de caractère
     * 
     * @see Class_For_JSON#Loi.java
	...
	/**
     * Renvoie le float correspondant à la propriété law:eco_effect pour un sujet "loi" passé en paramètre.
     * Assimilable à un "getter" JENA sur une propriété donnée avec un sujet passé en argument.
     * 
     * @param _rsc
     *            Une ressource "objet" JENA
     * 
     * Retourne un float correspondant à la valeur de la propriété law:eco_effect qui correspondant à l'effet économique d'une loi
     * 
     * @return Un float
     * 
     * @see Class_For_JSON#Loi.java
	...
	/**
     * Renvoie le float correspondant à la propriété law:life_effect pour un sujet "loi" passé en paramètre.
     * Assimilable à un "getter" JENA sur une propriété donnée avec un sujet passé en argument.
     * 
     * @param _rsc
     *            Une ressource "objet" JENA
     * 
     * Retourne un float correspondant à la valeur de la propriété law:life_effect qui correspondant à l'effet sur la qualité de vie d'une loi
     * 
     * @return Un float
     * 
     * @see Class_For_JSON#Loi.java
	...
	/**
     * Récupère le sujet du triplet RDF (loi_a_examiner, law:is_voted, sujet_voulu) sous la forme d'une chaine de caractère.
     * 
     * @param _st
     *            Une ressource JENA qui est un sujet "loi" 
     * 
     * Retourne la chaine de caractère correspondant à la valeur de la propriété is_voted pour le sujet loi passé en paramètre.
     * 
     * @return Une chaine de caractère
     * 
     *  @see getLoiFromParti
