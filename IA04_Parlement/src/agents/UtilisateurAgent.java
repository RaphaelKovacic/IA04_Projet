...
/**
 * <b>UtilisateurAgent est la classe représentant l'agent utilisateur dans notre SMA Parlement.</b>
 * <p>
 ...
 * <li>Un attribut Credibilite qui représente la crédibilité de l'utilisateur</li>
 * <li>Un attribut Parti_Politique qui stocke le parti politique de l'utilisateur</li>
 * 
 * <li>Un attribut L_Parti qui est une liste de tous les partis politiques possibles du jeu.</li>
 * 
 ...
 * Les comportements de l'agent KB sont spécifiés dans les huit classes suivantes
...
public class UtilisateurAgent extends Agent{
	...
	/**
     * La popularité de l'utilisateur. Variable.
     * Sa cote d'amour auprès du peuple
     * 
     * @see setup()
     */
	float Popularite;
	...
	/**
     * La notoriété de l'utilisateur. Variable.
     * Sa cote d'amour auprès des entreprises
     * 
     * @see setup()
     */
	float Notoriete;
     ...
     * 0 : simple député fraichement élu
     *  1 : quand attributs statiques sont >50
     *  2 : quand attributs statiques sont >65
     *  3 : quand attributs statiques sont >80
     *  4 : quand attributs statiques sont >90
     * 
     * @see setup()
     */
	...
	/**
     * Méthode d'instanciation (appelée à la création) de notre agent Utilisateur
     * <p>
     * Lors du lancement de notre plateforme JADE, l'agent Utilisateur est créé grâce à cette méthode setup()
     * </p>
     */
	...
	{ 
		...
		addBehaviour(new OneShotBehaviour(){
			...
			public void action() {
				...
				addBehaviour(new ChooseAlaw()); //Recevoir la liste des lois envoyée par mediateur afin de les afficher et d'en choisir une
	...
	/**
	 * <b>LActionsFromMediateur est le premier Behaviour de l'agent Utilisateur</b>
	 * <p>Il est de type Cyclic. Notre agent Utilisateur est en constante attente d'une requête PROPOSE de l'agent Médiateur avec les actions possibles de ce tour.
	 * </p>
	 * <p>
	 * Il implémente le comportement suivant :
	 * Affiche les actions possibles lors du tour de jeu en cours et la procédure pour les réaliser
	 * <p>
	 * 
	 * @author Benoit
	 * @version 2.1
	...
	/**
	 * <b>PrecisonActionFromMediateur est le second Behaviour de l'agent Utilisateur</b>
	 * <p>Il est de type Cyclic. Notre agent Utilisateur est en constante attente d'une requête REQUEST de l'agent Médiateur avec un complément d'insctruction à gérer par l'utilisateur concernant l'action d'un tour.
	 * </p>
	 * <p>
	 * Il implémente le comportement suivant :
	 * Afficher la procédure de changement de parti après que l'utilisateur ait choisi l'action de changer de parti.
	 * <p>
	 * 
			...
			if (message != null){
				switch(message.getContent()){
					...
					case "Change de parti": 
						...
						//Recrée la liste pour anticiper et gérer un éventuel prochain changement de parti
	 ...
	 * <p>Il est de type Cyclic. Notre agent Utilisateur est en constante attente d'une requête CONFIRM de du joueur avec le parti qu'il a choisi lors de l'action de changement de parti.
	 * </p>
	 * <p>
	 * Il implémente le comportement suivant :
	 * Récupère et modifie le parti de l'agent Utilisateur suite à l'action de changement de parti puis l'envoi du parti choisi par le joueur. C'est en quelque sorte la fin de l'action "changer de parti"
	 * <p>
	 * 
	 * @author Etienne
	 * @version 1.2
	 ...
	 * <p>Il est de type Cyclic. Notre agent Utilisateur est en constante attente d'une requête PROPOSE venant de l'agent médiateur et demandant au joueur de voter pour une loi.
	 * </p>
	 * <p>
	 * Il implémente le comportement suivant :
	 * Récupère la loi envoyée par l'agent médiateur et pour lequel l'utilisateur doit donner son vote.
	 * 
	 * Cette action de vote est obligatoire à chaque tour sauf si l'utilisateur choisi l'action "proposer une loi".
	 * 
	 * C'est cette action qui cloture le tour d'un utilisateur lorsqu'il n'a pas choisi de proposer une loi de lui même.
	 * <p>
	 * 
	 * @author Benoit
	 * @version 1.2
	 ...
	 * <p>Il est de type Cyclic. Notre agent Utilisateur est en constante attente d'une requête QUERY_REF venant de l'agent médiateur et demandant à cette agent de fournir ses informations.
	 * </p>
	 * <p>
	 * Il implémente le comportement suivant :
	 * Fourni les informations sur lui même (info statiques) lorsqu'il le médiateur le demande.
	 * 
	 * Cette action est très importante lorsque l'utilisateur choisi de demander l'avis du parlement ou de proposer une loi puisque pour être votée ou sondée, une loi doit contenir les informations du proposant. Dans le cas de actions nommées ci avant ce sera les informations de l'utilisateur.
	 * <p>
	 * 
	 * @author Etienne
	 * @version 1.2
	...
	class GiveYourInfo extends CyclicBehaviour{
		...
		public void action() {
			...
			// On attend la reception d'un message de type QUERY_REF venant de l'agent médiateur pour lui donner nos informations
			MessageTemplate mt = MessageTemplate.and(
			...
			if (message != null){
				...
				switch(message.getContent()){
					...
					//On créé une loi temporaire dans laquelle on va stocker nos informations propres qui sont importantes
					List<String> l_PartiPolitique = new ArrayList<String>();
					l_PartiPolitique.add(Parti_Politique);
					Loi utilisateur_carac = new Loi(0, null, 0, 0, l_PartiPolitique, "Utilisateur", Influence, Charisme, Popularite, Notoriete);
					
					//On sérialize cette loi que l'on envoie dans le message INFORM REF en réponse, à Médiateur
					ObjectMapper mapper = new ObjectMapper();
	 ...
	 * <p>Il est de type Cyclic. Notre agent Utilisateur est en constante attente d'une requête QUERY_IF venant de l'agent médiateur et demandant à l'agent utilisateur d'afficher les loi disponibles pour la proposition de loi ou la demande d'avis du parlement.
	 * </p>
	 * <p>
	 * Il implémente le comportement suivant :
	 * Affiche à l'utilisateur les lois disponibles au vote ou à la demande d'avis et fournit la procédure à celui ci pour effectuer l'une des deux actions.
	 * 
	 * Cette action d'affichage termine la première partie des actions : Proposer une loi ou Avis du parlement.
	 * <p>
	 * 
	 * @author Etienne
	 * @version 1.6
	...
	class ChooseAlaw extends CyclicBehaviour{
		...
		public void action() {
			...
			// On attend la reception d'un message de type QUERY_I venant de l'agent médiateur pour afficher les lois possibles
			MessageTemplate mt = MessageTemplate.and(
			...
			if (message != null){
					...
					//Stocke les lois envoyées par Mediateur dans liste de lois locale
					List<Loi> Loi_a_choisir = new ArrayList<Loi>();
					
					try {
						Loi_a_choisir = new ObjectMapper().readValue(content, new TypeReference<List<Loi>>() { });
						//Tri des lois à afficher sur l'id de celle ci.Affiche le plus petit ID en premier
						...
						if(message.getConversationId().equalsIgnoreCase("Proposition de loi"))
							System.out.println("Envoyer un message de type INFORM_IF à l'agent médiateur avec seulement l'ID de la loi choisie et 'Proposition de loi' en conversation-id.");
						else
	...
	/**
	 * <b>RequestToModifCara est le septième Behaviour de l'agent Utilisateur</b>
	 * <p>Il est de type Cyclic. Notre agent Utilisateur est en constante attente d'une requête INFORM venant de l'agent loi et demandant à l'agent utilisateur de modifier ses caractéristiques interne suite au votre d'une loi (soit qu'il a proposé ou bien qu'il a voté).
	 * </p>
	 * <p>
	 * Il implémente le comportement suivant :
	 * Mets à jour les informations internes de l'agent utilisateur après un vote.
	 * 
	...
	/**
	 * <b>ActualLevelOfUser est le huitième et dernier Behaviour de l'agent Utilisateur</b>
	 * <p>Il est de type Cyclic. Notre agent Utilisateur est en constante observation de l'agent utilisateur afin de modifier son niveau losque ses caractéristiques varient et entrainent un changement de niveau.
	 * </p>
	 * <p>
	 * Il implémente le comportement suivant :
	...
	class ActualLevelOfUser extends CyclicBehaviour{
		...
		public void action() {
			...
			//TODO Discuter des niveaux et de leur signification + des messages à afficher
			...
			//TODO Fin du jeu ? Envoie d'un message à l'agent simulation pour finir le GAME ?

		}
	}
	
	/**
     * Vérifie les informations de l'utilisateur afin qu'elles ne descendent jamais dans les négatifs.
     * 
